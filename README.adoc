...
TIP: For example, check that the database name and its details are mapped in the ConfigMap used by Mobile Security Service application and database. In this, way for example to connect to the database with the default values you may use inside of its contained the command: `psql -h localhost -U postgresql mobile_security_service

NOTE: To test/check it in the container with the default values you can
