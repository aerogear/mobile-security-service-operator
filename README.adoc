...
Update the cluster host IP in the following CRS.

* link:./deploy/crds/mobile-security-service_v1alpha1_mobilesecurityservice_cr.yaml[MobileSecurityService CR]
* link:./deploy/crds/mobile-security-service_v1alpha1_mobilesecurityservicebind_cr.yaml[MobileSecurityServiceBind CR]
* link:./deploy/crds/mobile-security-service_v1alpha1_mobilesecurityservicemonitor_cr.yaml[MobileSecurityServiceMonitor CR]
...
NOTE: It is planned get this value automatic in th future.
...
=== Watch specifications for the MobileSecurityServiceMonitor (CR)

The MobileSecurityServiceMonitor watches the binds in the cluster in order to delete the data in the Mobile Security Service by it REST API. The link:./deploy/crds/mobile-security-service_v1alpha1_mobilesecurityservicemonitor_cr.yaml[MobileSecurityServiceMonitor CR] has properties which follows that defines where it should looking for by binds in the cluster.

[source,shell]
----
  # The following fields are selectors for the Monitor looking for by the binds and be able to unbind apps in the Service
  # by the REST Service Api. If this values be empty then the Monitor will check all cluster. Also, the combination of both
  # are valid as well.
  # ----
  # Add a namespace where the bind app are made in oder to allow monitor check the bind applications and to do the unbind
  namespaceSelector:
  # Add label selectors for the Monitor find the bind apps and be able to do the unbind
  labelSelector:
    matchLabels:
      app: mobilesecurityservic
----
...
| link:./deploy/crds/mobile-security-service_v1alpha1_mobilesecurityservicebind_cr.yaml[MobileSecurityServiceBind]             | CR which specified the app name and id. It is used to create and update the app in the Service REST API.
| link:./deploy/crds/mobile-security-service_v1alpha1_mobilesecurityservicemonitor_cr.yaml[MobileSecurityServiceMonitor]             | CR which watch the bind in order to delete the app by the Service REST API.
|===
